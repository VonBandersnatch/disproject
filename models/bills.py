...
def list_bills():
    conn = db_connection()
    cur = conn.cursor()
    cur.execute('SELECT ps.parti, ps.stemme, s.titelkort FROM PartiStemmer ps JOIN Afstemning a ON ps.afstemningsid = a.id JOIN Sagstrin st ON a.sagstrinid = st.id JOIN Sag s ON st.sagid = s.id')
